#!/usr/bin/perl
 ...
 use strict;
 use vars qw ($str $parser $debug $pidfile @webbots $j @oscar @connecting @connected @commit_buddylist
              ...
              $portnumber $securitycode $connectedtomsg $temp %webusers $secure $loginurl $t $iminurl);
 ...
 $iminurl="http://clearimageonline.com/cgi-bin/apps/start/livetalk/imin.html";
...
sub im_in {
 ...
 my $me;
 my $islivetalk;
 $me=lc($oscar->{screenname});

 print "IM_IN----BEGIN----\r\n";
 print "$me:$sender:$is_away:\r\n$message\r\n";
 print "IM_IN----END----\r\n";
...
# $message =~ tr/  / /s;
 ...
 for ($j=0; $j<=$#bridges; $j++) {
  if (lc($oscar->{screenname}) eq lc($bridges[$j][1]) and lc($sender) eq lc($bridges[$j][0])) {
   ...
   if (lc($b0) eq lc("SET") or lc($b0) eq lc("GET") or lc($b0) eq lc("LOGIN") or lc($b0) eq lc("HELP")){
    if (lc($b0) eq lc("SET")) {
     if ($b1 ne "") {
      $webusers{$b1}=$b2;
      open (CN, '>webusers.csv');
    ...
    if (lc($b0) eq lc("HELP")) {
     sendim($oscar,$sender,"SET id name<br>GET id<br>LOGIN<br>BREAK<br>Use any of these commands above to get information regarding LiveTalk.");
    }
...
#     $oscar->send_im ($sender,$dirty);
      ...
      sendim($oscar,$sender, "unknown sender");
...
#     $oscar->send_im ($sender, "DELIVERY FAILURE CAN'T OPEN chats/comments$bridges[$j][3].txt");
  ...
  if ( $islivetalk eq 'FALSE' ) {
   $str="$b0 $b1 $b2";
   $str =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
   my $content = get "$iminurl?message=$str&sender=$sender&me=$me";
   sendim($oscar,$sender,$content);
  }
...
sub buddylist_error {
 ...
 print "[LiveTalk Server is running on port $portnumber]\r\n"; 
...
#      $oscar[$i]->send_im ($b1, $b2);
       ...
       $str=$oscar[$i]->{screenname};
       print $client "IMSEND: Sent via $str\r\n";
      }
     }
     if ($inoscar eq "FALSE") {
